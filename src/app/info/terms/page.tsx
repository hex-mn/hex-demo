...
export default function Page() {
    ...
    useEffect(() => {
        const getData = async () => {
            const response = await sendRequestToPublicAPI("GET", `/tos/get/`);
            setPageContent(response.tos || "");
            setLoading(false);
        };
        getData();
    }, []);
    ...
    return (
        <div className="prose prose-slate max-w-3xl w-full text-lg md:text-xl text-gray-800 dark:prose-invert dark:text-gray-100 animate-fade-in mx-auto py-8 px-4">
            <h1 className="text-2xl md:text-3xl font-bold mb-8 text-center">Үйлчилгээний нөхцөл</h1>
